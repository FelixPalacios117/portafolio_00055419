...
int poblaciontotal(string busqueda){
    ...
    for(int i=0;i<25;i++){
        if(busqueda==continentesnuevos[i].nombre){
            ...
            for(int j=i;j<i+5;j++){
                suma+=continentesnuevos[j].habitante;
            }
            i=26;
  	}
  	else{
...
int main()
{
 ...
 double promedio=0;
    ...
    while(!continentencontrado){
        ...
        cin>>nombrecontinente;
       cout<<"La suma de la poblacion del continente : "<<nombrecontinente<<" es  : "<< poblaciontotal(nombrecontinente)<<endl;
    }
for(int i=0;i<25;i++){
if(continentesnuevos[i].habitante>cantidad)
{
    cantidad=continentesnuevos[i].habitante;
    capital=continentesnuevos[i].capital;
}
promedio+=continentesnuevos[i].promedioedades;
}
cout<<"La capital con mayor poblacion es : "<<capital<<endl;
